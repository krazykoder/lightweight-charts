...
## Line and Area Series Line Types

Line and Area series support a `lineType` option, which controls how the line is drawn:

- `LineType.Simple` (default): A standard straight line between points.
- `LineType.WithSteps`: A stepped line, connecting points with horizontal and vertical segments.
- `LineType.WithGaps`: A line that visually breaks at gaps in the data (e.g., where the value is `null` or `undefined`).
...
series.setData([
    ...
    { time: '2020-01-02', value: null }, // gap
...
You can add markers to any series using the `setMarkers` method. The following shapes are supported:
...
```js
series.setMarkers([
    { time: '2020-01-01', position: 'aboveBar', color: 'blue', shape: 'triangleUp' },
    { time: '2020-01-02', position: 'belowBar', color: 'red', shape: 'triangleDown' },
]);
```

These new options allow for more flexible visualizations, especially when working with discontinuous data or custom marker requirements.
